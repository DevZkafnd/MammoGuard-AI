...
function AdminSidebar({
  ...
  const menuItems = [
    { label: "Admin IT", active: false },
    { label: "Manajemen Akun", active: true },
    { label: "Manajemen Model AI", active: false },
  ];
      ...
      <div className="px-6 py-6">
        <div className="flex items-start gap-3 rounded-[12px] px-3 py-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] uppercase tracking-[0.15em] text-[#8a95a1] font-medium">
              AI DIAGNOSTICS
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#0a5c4f] to-[#0d7a68] text-[14px] font-bold text-white shadow-sm">
              {session.nama.split(' ').map(n => n[0]).join('').toUpperCase()}
            </span>
            <div className="min-w-0">
              <p className="truncate text-[14px] font-semibold text-[#2a3949]">
                {session.nama}
              </p>
              <p className="truncate text-[11px] text-[#8a95a1] font-medium">{session.peran}</p>
            </div>
          </div>
